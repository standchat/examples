...
def measure_reach(objs, data):
    """The box Gilly's animation sweeps through, in the USDZ's own space (meters, Y up,
    Gilly facing +Z), overall and per segment. On Vision Pro, Safari clips a model at
    the page surface, so ar.js keeps the nearest point of any mood behind it."""
    scene = bpy.context.scene
    ...
    for f in range(n):
        ...
        for o in objs:
            ...
            corners = np.array([ev.matrix_world @ Vector(c) for c in ev.bound_box])
            usd = np.stack([corners[:, 0], corners[:, 2], -corners[:, 1]], axis=1)  # Blender Z up → USD Y up
            lo[f] = np.minimum(lo[f], usd.min(axis=0))
            hi[f] = np.maximum(hi[f], usd.max(axis=0))
    scene.frame_set(0)
    fps = data['fps']
    segments = {}
    for seg in data['segments']:
        a, b = int(round(seg['start'] * fps)), min(n, int(round(seg['end'] * fps)))
        segments[seg['name']] = (lo[a:b].min(axis=0), hi[a:b].max(axis=0))
    reach = (lo.min(axis=0), hi.max(axis=0))
    log('reach', np.round(reach[0], 4), np.round(reach[1], 4), 'rest front', round(float(hi[0][2]), 4))
    for name, (a, b) in segments.items():
        log(f'  {name:7s} front {b[2]:.4f}  top {b[1]:.4f}  bottom {a[1]:.4f}')
    return reach, {name: float(b[2]) for name, (a, b) in segments.items()}
...
def write_segments(data, reach, fronts, path):
    lines = [
    ...
    for s in data['segments']:
        lines.append(f"  {s['name']}: {{ start: {s['start']:.4f}, end: {s['end']:.4f}, loop: {str(s['loop']).lower()}, front: {fronts[s['name']]:.4f} }},")
    lines.append('};')
    lines.append(f"export const DURATION = {len(data['frames']) / data['fps']:.4f};")
    lines.append("// The box all of the moods stay inside, in the same space.")
    fmt = lambda v: '[' + ', '.join(f'{x:.4f}' for x in v) + ']'
    lines.append(f"export const REACH = {{ min: {fmt(reach[0])}, max: {fmt(reach[1])} }};")
...
def main():
    ...
    if ARGS.get('usdz'):
        ...
        reach, fronts = measure_reach([skin_mesh, *eye_objs], data)
        export_usdz(os.path.join(ARGS['out'], 'gilly.usdz'), [rig, skin_mesh, *eye_objs])
        write_segments(data, reach, fronts, os.path.join(ARGS['out'], 'gilly-segments.js'))
